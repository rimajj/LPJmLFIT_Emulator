...
| ~~7~~ | **RESOLVED — `mtry` dilution is a MINORITY of the effect, and it saturates** (`[VERIFIED]`, jobs `1680827`/`1681596`/`1683694`). Raising `mtry` at ncond 14 restores the driver-touch probability from 0.790 to 0.965 (m7) / 0.985 (m8). **Level:** Wooddens `emu_r` goes 0.9095 → 0.9124 → **0.9121** — a +0.003 gain that then *saturates* (m7 ≈ m8, within §2's 0.004–0.006 hash sd), i.e. 7 % of the +0.0402 env gain, and it never approaches the p8 arm's 0.8693. **Transient amplitude:** `\|Ra−1\|` goes 0.1306 → 0.1039 → 0.1052 against p8's 0.0728, recovering **46 %** at m7 and then saturating. So the amplitude cost of §5.2 is **~46 % dilution and ~54 % the env columns' content**, and **no `mtry` setting recovers p8's amplitude fidelity** — the cost is substantively attributable to the tail, not to a knob. `Rr` at hash folds *rises* slightly with `mtry` (0.4146 → 0.4230), so the hash-fold `Rr` advantage is not a dilution artifact either; the honest finding remains its reversal under blocking, for which no blocked m7/m8 arm exists. | A blocked `p14env` rung at `MTRY=7` would test whether the blocked `Rr` reversal is also partly dilution — the one remaining `mtry` question. | 1 rung |
