...
| 7 | **`mtry` dilution is NOT the explanation for the LEVEL story** (`[VERIFIED]`, job `1680827`): raising `mtry` 4 → 7 at ncond 14 — restoring the driver-touch probability from 0.790 to 0.965 — buys only **+0.0029** Wooddens `emu_r` (0.9095 → 0.9124) and +0.0092 `sd_ratio`. That is 7 % of the +0.0402 env gain and 14 % of the −0.0201 width penalty, so neither is an `mtry` artifact. Whether it explains §5's **transient** amplitude cost is still open. | `MTRY=8` (matched *probability*, 0.985) is job `1681596`, and a response gate on both mtry arms is chained `afterok` as job `1683694`. If `\|Ra−1\|`/`Rr` move back toward the p8 arm as `mtry` rises, the transient cost is dilution and is knob-fixable; if not, it is the columns' content and only a transient tail can help. | 0 — in flight |
