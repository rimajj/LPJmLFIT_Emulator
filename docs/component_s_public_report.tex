...
One honest gap should be flagged here: unlike the individual traits discussed next, there is currently no equivalent ``natural noise-floor ceiling'' computed for the tree-count prediction (the seed1-versus-seed2 comparison described in Section 4.2 has not yet been built out for counts). This means that while an R-squared of roughly 0.982 is very high in absolute terms, we cannot yet say precisely how close this is to the best any model could possibly do, the way we can for the individual traits below. This is explicitly listed as still-open work, not a result being hidden.
...
\caption{The real, simulated global map this section is trying to reproduce: the average number of living trees per simulated forest plot (``patch''), for every one of the world's grid cells, as actually produced by the detailed physical simulator over the historical period. Brighter yellow means more trees per plot; dark purple means very few; white areas are locations with no living trees recorded at all (deserts, ice, and similar).}
...
\caption{Component S's own prediction for the exact same quantity, location, and time period as Figure \ref{fig:count_observed} -- but every single grid cell shown here was held out of that particular prediction's training data (Section 4.1), so this is a genuine out-of-sample forecast, not a fit to data the model already knew the answer to. Side by side with Figure \ref{fig:count_observed}, the two maps are visually almost indistinguishable, which is the visual counterpart of the 0.9826 held-out-by-location R-squared reported in Table \ref{tab:counts}.}
...
\caption{The difference between the two previous maps at every location (predicted minus observed), on a scale centered at zero: white means no detectable error, one color means the model over-predicts tree density there, the other means it under-predicts. Most of the globe is white or very pale, consistent with the high R-squared in Table \ref{tab:counts}; the clearest exceptions sit right along the boreal tree line, exactly the kind of sharp ecological boundary that is intrinsically hard for any model to place with pixel-perfect precision.}
...
\caption{The full distribution (the entire collection of values, not just an average) of tree counts per patch, real versus predicted, pooled across every held-out patch-year in the historical period (the vertical axis is on a logarithmic scale, meaning each gridline up represents a tenfold increase, so both common low counts and rare high counts stay visible on the same plot). The two curves overlap closely across most of the range; the model's curve runs slightly narrower than reality's in places, because Component S reports the single most likely outcome for each situation, whereas the real data also includes natural patch-to-patch randomness on top of that average.}
...
\caption{Each of the 53,699 dots is one grid-cell location: its position along the bottom axis is the real, observed average tree count at that location (averaged over all its simulated patches and years); its height is Component S's held-out predicted average for that same location. The dashed diagonal line marks perfect agreement. Nearly every dot sits almost exactly on that line across the full range from sparse (bottom left) to dense (top right) forest, which is the per-location R-squared of 0.9988 reported in Table \ref{tab:counts} made visible.}
...
\caption{The overall, whole-world distribution of each trait, real versus predicted, pooled across every held-out tree in the historical period. Four panels are the traits actually used by the running model (specific leaf area, wood density [``Wooddens''], rooting depth [``D95max''], drought tolerance [``minwscal'']); the two panels marked ``[diag]'' are the diagnostic-only biomass and height axes from Section 3.2.3. Every panel reports its interquartile-range-normalized error and its Kolmogorov-Smirnov value; both are small for every trait, and the two curves sit almost on top of each other in all six panels. This is a whole-population comparison, not a location-by-location one -- it confirms Component S has learned the right overall shape for every trait globally, but the next two figures are needed to check any single real-world location. The above-ground-biomass panel's interquartile-range-normalized error (about 0.64) looks alarmingly large purely because that trait is heavy-tailed (Section 5.5's caveat); its Kolmogorov-Smirnov value (about 0.01) is the trustworthy number for that one panel, and it is small.}
...
\caption{Now zoomed in to individual locations: each dot is one of 52,516 grid cells, comparing the real median value (the middle value once every tree's trait value at that location is lined up from smallest to largest) of a trait at that location against Component S's held-out predicted median for that same location, with the dashed line marking perfect agreement. Drought tolerance, biomass and height hug the diagonal tightly across their full range. Wood density (top center) is visibly the noisiest of the four live traits, matching the largest gap and lowest prediction-spread ratio already flagged numerically in Table \ref{tab:traits}: locations with genuinely very dense wood are, on average, somewhat under-predicted.}
...
\caption{The same per-location comparison, mapped back onto the globe and measured with the Kolmogorov-Smirnov statistic computed separately at every grid cell, showing \emph{where} on Earth each trait's full local distribution is reproduced well or poorly. Pale colors mean a good local match; dark patches mean a poorer one. Across all six traits, most of the world's land surface shows a good local match; the harder-to-match patches recur in the same handful of regions across several traits at once, suggesting these are places where Component S's current set of environmental input clues (Section 2.5) least fully captures the local forest composition, rather than unrelated random noise.}
...
\caption{Each dot is one grid cell, comparing the simulator's own true total stand biomass (bottom axis) against the combined prediction described above -- predicted tree count multiplied by predicted biomass per tree (left axis), with the dashed line marking perfect agreement. The historical panel hugs the diagonal tightly, matching the per-location R-squared of 0.931 in the text; the future-climate panel shows more scatter around the line, matching the wider cell-to-cell spread (30.7 percent of cells more than 10 percent off) already flagged in the text above.}
...
\caption{The global map of total stand biomass, real versus predicted, over the historical period -- the same kind of side-by-side comparison as Figures \ref{fig:count_observed}--\ref{fig:count_bias}, but for biomass rather than tree count. The overall geographic pattern (dense biomass in the tropics and temperate forests, very little in deserts and tundra) is reproduced closely.}
...
A model's accuracy numbers are only as trustworthy as the process that produced them. This section describes three separate mistakes the project caught in its own work, presented deliberately as evidence of a rigorous, self-skeptical validation culture -- catching and fixing a mistake before it becomes a permanent, uncorrected claim is exactly what a trustworthy scientific process is supposed to look like, not something to be embarrassed about.
...
None of these open items call into question the core, already-validated results in Section 6 within the scope those results actually cover -- the tree-count model and two of the four live traits (specific leaf area and drought tolerance) are performing close to their respective achievable ceilings in the offline, one-year-at-a-time checks reported here, and the formal pass/fail basis check across all six measured axes currently passes. The open work is a matter of pushing the two weaker trait axes (wood density and rooting depth) further, extending coverage to grasses, and building out the recursive, coupled-run evidence that complements today's offline numbers -- not a sign that the overall approach is unproven.
...
Tested the hard way -- on grid-cell locations, and separately on entire climate periods, the model has never been allowed to see during training -- Component S currently predicts tree counts and two of its four core live traits (specific leaf area and drought tolerance) at a level close to the best that is mathematically achievable in this offline, one-year-at-a-time sense, given that even two runs of the original physical simulator do not perfectly agree with each other. Wood density and, to a lesser extent, rooting depth remain the clearest areas for improvement, and the project has stated, in concrete and measurable terms, exactly what closing the wood-density gap would look like. Component S today covers trees only, and its numbers describe single-year, offline accuracy rather than proven behavior over many years chained together inside a live coupled simulation -- both stated here as open work, not as fine print to be overlooked. Three separate instances of the project catching its own mistakes -- an incomplete tree-type filter, a scientifically plausible-sounding but mathematically incorrect explanation, and a subtle data-processing inconsistency -- are presented here not as embarrassments but as the visible evidence of a genuinely rigorous, self-checking process, which is ultimately what should give a reader confidence in the numbers this report presents.
