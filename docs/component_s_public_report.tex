...
\addlinespace
\textbf{Not established (measured, negative)} & Recursive stability inside a live coupled run. The stand's tree density has no level anchor: it is advanced by predicted year-on-year \emph{ratios}, so the count model's absolute skill never reaches it. A fourfold difference in starting density is still a fourfold difference after 300 years of identical climate -- the discrepancy converges to a constant rather than decaying. Errors in the level therefore accumulate without a restoring force. \\
\bottomrule
\end{tabular}
\end{table}
...
The objective is a land component that runs online inside an Earth System Model, reproducing LPJmL-FIT's forest dynamics faithfully at a small fraction of its cost. Five steps remain, in priority order.

\paragraph{1. Give the recursive stand a level anchor.}
This is now the first priority, ahead of the warming-response gap, because it compounds without bound and because no amount of predictive skill fixes it. The count model predicts an \emph{absolute} number of trees, and predicts it well. The coupled loop uses only the year-on-year \emph{ratio} of that prediction and advances the stand multiplicatively, which is what allows a per-patch count to drive a cohort-density stand without knowing the patch area. The cost of that convenience is that the stand's absolute level is never stated: it is the initial value multiplied by a product of ratios, with no restoring force. Measured directly, a fourfold difference in initial density is still a fourfold difference after 300 years of identical forcing; the discrepancy is transiently \emph{amplified} to sevenfold around year 25 and then settles back to a constant fourfold rather than decaying away. Separately, the diagnostic shows the internal predicted count converging to a common attractor while the physical stand those same runs carry does not. This also explains a previously unattributed result from the coupled line: feeding the reference model's true count back in each year removes only about two-thirds of the coupled count error, because doing so repairs the ratio and leaves the level untouched.

The fix is specified: carry the count-to-density conversion across the component interface and blend the multiplicative update toward the absolute target with a stated relaxation time, rather than replacing it outright. It is not a local change -- it modifies an interface contract between two components and moves every coupled baseline -- which is why it is scoped rather than done.
...
\paragraph{3. Complete the ceiling measurement.}
...
Its warming response is damped by about 40 percent, its recursive behaviour inside a live coupled run is now measured and is the larger of the two problems, and it covers trees only. Neither route is yet a solved one: the response gap has had its most promising candidate measured and refused, and the recursive defect has been localised to a missing level anchor. On present evidence Component S is ready to supply forest structure to a coupled land model, and not yet ready to be the basis of a quantitative claim about the magnitude of forest change under warming.
