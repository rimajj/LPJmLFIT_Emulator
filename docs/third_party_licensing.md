...
|---|---|---|---|
| **LPJmL-FIT v5.6.004** (`/home/jamirp/lpjml56fit`, © 2007–2025 PIK) | The model this component is derived from; the C binary is our numerical-regression **oracle** and daily data generator | The physics: photosynthesis→GPP→NPP, the λ solve, PET/ET, soil water/snow, respiration, allometry, mortality, establishment. Parameter values are the C-source values. Also the **soil-texture table** (`par/soil_20m.js` `soilpar` sand/silt/clay) + the ground-truth run's per-cell soilcode map, used to prescribe the ONLINE Terrarium soil (ADR 0083) | `src/{fdiff,allometry,state,climbuf}.jl`, `src/components/{fast,slow}.jl` headers · `scripts/online_coupling/build_soil_texture_field.py` header · `CITATION.cff` · `refs.bib` |
| **Terrarium.jl** (NumericalEarth / TUM-PIK-ESM) @ `4f42508`, v0.1.3 | Differentiable land-surface substrate; the **P4 coupling substrate** | Its `Abstract*` process interfaces and the `speedy_{dry,wet}_land.jl` coupling pattern — S/F/E sit *behind* its interfaces. Additionally **cross-read only** (no code, no dependency) for component E's two-layer ground heat: its half-cell `ImplicitSkinTemperature` (`Ts = Tg − G·Δz/2κₛ`) and conduction column | (P4, pending) `ext/` header · `src/components/energy.jl` header (ADR 0074) · `CITATION.cff` · `refs.bib` |
| **SpeedyWeather.jl** (incl. `RingGrids`, `SpeedyWeatherInternals` — same monorepo) | The atmosphere for the **online** coupled run | `SpectralGrid` / `RingGrids` / `LandModel` wiring; `PrescribedLand{Heat,Humidity}Flux` for H/LE injection. Additionally **cross-read only** for ADR 0074: its `LandBucketTemperature` implements the same MITgcm two-layer equations, and its `LandThermodynamics` carries the published soil constants we adopt | (P4, pending) `ext/` header · `src/components/energy.jl` header (ADR 0074) · `CITATION.cff` · `refs.bib` |
| **MITgcm land package** (two-layer soil; `mitgcm.readthedocs.io/en/latest/phys_pkgs/land.html`, users guide §8.5.2.2 as cited by SpeedyWeather.jl) | The two-layer prognostic soil-temperature formulation LPJmL-FIT and component E both lacked | ⚠️ **Method from the documentation, INDEPENDENTLY implemented** — the `T1`/`T2` forward update, the inter-layer conductance `Δ = 2λ/(z1+z2)`, and the published thermal constants (`λ_soil` 0.42 W/m/K, `C_dry` 1.13e6, `C_water` 4.2e6 J/m³/K, `γ` 0.3, `z2` 2.0 m). **No code copied.** Our `z1` = 0.75 m deliberately differs from MITgcm's 0.2 m: 0.2 m is under-resolved at our daily step (ADR 0074 §2) | `src/components/energy.jl` header (ADR 0074) · `CITATION.cff` · `refs.bib` |
