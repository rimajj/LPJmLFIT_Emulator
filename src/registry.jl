...
const COMPONENTS = [
    Component(id=:S, name="Slow distribution emulator", kind=:ml, timescale=:annual,
        description="p(traits,size | drivers,state) + count N; allocates delivered NPP (flux-then-integrate)"),
    Component(id=:F, name="Fast physical core (LPJmL-FIT)", kind=:physics, timescale=:daily,
        description="photosynthesis→GPP→NPP, water balance, snow, soil thermal; conserving"),
    Component(id=:E, name="Energy-balance + skin-T closure", kind=:hybrid, timescale=:subdaily,
        description="solve T_skin; partition A=Rn−G into LE,H,G (H residual); reuse Terrarium.jl"),
    Component(id=:ATM, name="Atmosphere (ESM)", kind=:external, timescale=:subdaily,
