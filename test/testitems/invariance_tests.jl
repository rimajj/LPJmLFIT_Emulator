...
@testitem "Invariance" tags = [:invariance] begin
    ...
    for _ in 1:2000
        ...
        @test isapprox(softmax_partition(v), softmax_partition(v .+ c); atol = 1.0e-9, rtol = 1.0e-7)

        a = 100 * randn(rng)
        et = 50 * randn(rng)
        @test isapprox(latent_heat(a * et), a * latent_heat(et); rtol = 1.0e-9, atol = 1.0e-6)
    end
