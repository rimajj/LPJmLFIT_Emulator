...
**1. THE CRITERION, MET.** (a) the below-ground wood growth port landed (ADR 0132) — was the blocking
condition. (b) mean `|bmi_F/bmi_C − 1|` over the four readable cells **0.1914 → 0.1581** on the
pre-registered arm pair `P→Pg`, and **0.1599 → 0.1266** on the growth arms `Pbgg→Pgbgg` that now exist —
**the same −0.0333 in both**, so the gate's assimilate effect is additively separable from the below-ground
port's and the partial cancellation condition (a) existed to protect against does not appear in this
channel. Including the excluded fifth cell the mean still falls, so the exclusion is not doing the work.
(c) the default was flipped **alone, with no baseline touched**, and the blast radius measured:
**4 assertions of 275 597** (`logs/M-treegate-flip.1771606.out`) — third flip in a row at 3–4. Two were the
"default is off" assertions themselves; the other two are the deliberate moves below. Suite green after:
**275 605 pass / 0 fail** (`logs/M-treegate-flip2.1771985.out`), docs built locally exit 0 with all five
mermaid fences rendering (`logs/M-docs0133.1772109.out`).

**2. THE TWO BASELINE MOVES, each regenerated by a harness that reproduces the OLD numbers in the same
run.** Hainich canopy `gpp_annual` 1250.124 → 1237.437 (−1.02 %) — and **only that row**: the four water
rows come out at ratio **exactly 1.0**, which is the mechanism's own prediction (the gate multiplies tree
GPP and tree `rd`, both formed *after* `gp_stand`, transpiration and the per-layer withdrawal), so it is a
check, not a coincidence. Coupled 5-cell pins re-measured with a new **`TREE_GATE=0`** opt-out arm on
`biome_ensemble_pin_probe.jl` that reproduced **all ten previously committed pins to every printed digit**.

**3. ⚠ THE COST, and it must travel with the gain.** The photosynthesis channel gets **WORSE**:
mean `|gpp_F/gpp_C − 1|` **0.0570 → 0.0611**, entirely at `semiarid_sahel` (0.9065 → 0.8545, a deficit this
DEEPENS) while boreal 1.0273 → **1.0022** and Hainich 1.0851 → 1.0744 improve and the Amazon is unmoved.
Coupled stand cover falls **3.1 %** at boreal and **1.0 %** at Hainich. The respiration channel improves
(`|cue_F/cue_C − 1|` 0.1420 → 0.1363). This is ADR 0131 §1's warning realised: the gate moves the two
halves of the ADR 0129/0130 split in OPPOSITE directions. It is flipped anyway because the criterion was
pre-registered on the assimilate — the quantity that drives growth — and because the gate is what the C
does, not a tuning knob.

**4. THE SAHEL'S COUPLED GPP RISES 1.6 % AND THAT IS NOT A CONTRADICTION.** Within a day the gate can only
LOWER GPP (asserted). Over two coupled years it stops F paying leaf respiration against a collapsed
assimilation on drought days, so that cell's tree carbon balance improves (ADR 0131: −83.8 → +34.6
gC/m²/yr), the canopy GROWS instead of shrinking, and a bigger canopy assimilates more. A within-day
monotonicity and a multi-year mean are different statements about the same operator.

**5. 🔎 TWO PROCESS FINDINGS, both about this line's own recent work.** (i) **Four probes had control arms
the flip would have silently relabelled.** Worst case, and it would have been silent:
`biome_sapwood_bg_probe.jl` builds its gated arm by copying its ungated one and setting the flag ⇒ with the
default moved, `PARAMS_TG` would have been field-for-field equal to `PARAMS`, collapsing **`Ag ≡ A` and
`Pg ≡ P`**, and 30 committed rows of the growth-channel decomposition would have been reproducible only
under labels that no longer described them. Fixed in that probe plus `biome_canopy_growth_probe.jl`,
`biome_slow_oracle_probe.jl`, `biome_resilience_probe.jl`. **The rule: an arm that MEANS a specific value
passes that value; only an arm that means "whatever ships" takes the default by omission** — and the pin
probe is deliberately the latter, which is why it needed an opt-out KNOB instead of an explicit value.
(ii) **A guardrail-4 assertion written one week earlier had become a green test that proved nothing** —
"the default reproduces the opt-out bit-for-bit" kept passing after the flip because at the soft default
sharpness the gate's sigmoid saturates to exactly 1.0 on every day of that fixture's forcing. Kept,
relabelled as a property of the FIXTURE, with the wiring proven where the gate actually fires.

**6. AN HONEST LIMIT recorded in the fixture's own header rather than smoothed over.** The canopy
regenerator's gate-OFF control reproduces the committed baseline to **1.3e-4, not to its printed digits**:
the four water rows carry ≤ 1.6e-4 of accumulated **sub-tolerance drift** from earlier physics changes that
the 1e-3 drift alarm never fired on. That drift predates this flip and was **left in place** — absorbing it
into this re-record would have hidden a pre-existing fact behind an unrelated change.

▶ **WHAT TO DO NEXT — in order.**

1. **`boreal_siberia`'s ALLOCATION, the one cell where it genuinely binds — and the concrete suspect is a
   per-PFT flag F does not have.** `AllocParams.is_deciduous` is `true` for **every** tree in F, while the C
   gates the summergreen full-leaf recycle (`leaf/1.05`) on `tree->isphen` (`turnover_tree.c:100`) — so F
   runs a summergreen leaf recycle for the boreal/temperate **evergreen** PFTs. This is the same shape as
   the per-PFT parameter work that is already landed and measured (ADR 0126), it is cheap, and boreal is the
   cell ADR 0132 confirmed the below-ground port is **not** the answer for (it closed 97 % of a bar its own
   measurement said was 89 % something else). The other suspect at that cell is the `reprod_cost` path.
2. **THE ASSIMILATE REMAINS THE HEAD OF THE F QUEUE, and the balance of the split has shifted.** At Hainich
   F still over-grows by 57 % and ADR 0127 §4 put **77 %** of that on the assimilate; ADR 0130 split the
   assimilate ≈43-47 % photosynthesis / ≈57-53 % respiration. The respiration half now has two priced leads
   landed (per-PFT `respcoeff`, and this gate at 17.5 %) — so **the photosynthesis half is now the larger
   unexplored share**, and nothing on the F queue aims at it yet. That is the gap to scope next.
3. **`bg_growth`'s DEFAULT IS STILL BLOCKED ON LINE S** — unchanged, criterion already pre-registered:
   flip once S carries `heartwood_bg_c` through `slow.jl`'s four `TreePools` rebuild sites (recruit mix
   `:161`, `_with_nind` `:249`, recruit build `:479`, K-cap merge `:670`) **and** the full suite's failure
   list is enumerated. Raised in `lines/S/STATE.md`; nothing is broken today because the flag is off.
4. **THE DIFFERENTIATED PATH NOW RUNS THE GATE BY DEFAULT, at the SOFT sharpness — verified, not assumed.**
   `rollout_canopy_years_gpp` reads `p.water` with no reconstruction, so the trainer inherits
   `tree_demand_gate = true` at `βgpd_gate = 2e4` (AD-usable, licensed by ADR 0131 §6, which measured that
   sharpness reproducing the C's hard step to the printed digit at three of five cells). All four
   `nn_canopy_training_tests` Enzyme items pass unchanged. ⚠ Any trainer arm that meant "the pre-0133 path"
   must now say so explicitly. Separately unchanged from the last handoff: the trainer still builds its
   pools with the pre-`sapwood_bg` arity, so the below-ground sink is absent from the differentiated path.

### 0-PREV-17. ✅ DONE 2026-08-13 (session 17) — **THE BELOW-GROUND WOOD SINK IS PROGNOSTIC. IT PASSES
