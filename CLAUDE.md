...
| C binary (rebuilt, emits daily grass GPP/NPP) | `/home/jamirp/lpjml56fit/bin/lpjml` (pristine backup: `bin/lpjml.pre_dgrass.bak`). ⚠ **Rebuilt again 2026-08-11** to carry the opt-in rung-2 demography hooks (**`patches/lpjmlfit_rung2_hook_v5.patch`**, ADR 0061 → 0120 → 0121 → 0122 → 0123; the earlier `..._v4/_v3/_v2/_demography_hook` patches are kept only for the provenance of the binaries those ADRs gated). The hooks are **inert unless `LPJ_RUNG2_DIR` or `LPJ_RUNG2_APPLY_DIR` is set**, and the rebuild was gated: 139 decoded quantities + `globalflux` + `ind` identical to the previous build on a matched single-cell 2000–2019 run. Copy at `bin/lpjml_rung2_v5` (`bin/lpjml_rung2` is still the ADR-0061 build). ⚠ **With either env var set the binary DEFERS its demographic kills to the end of the growth loop** (ADR 0123): that is what lets the external demography spare a tree, it is mathematically inert but not bit-identical, and it moves the C's own trajectory by 0.05 % of stem-years over 20 yr at one cell. Disclose it with any rung-2 number; with both vars unset the model is stock. **After ANY rebuild, run `scripts/diagnose_cbinary_rebuild_equality.py` before quoting a C-vs-emulator number** — and do NOT use `cmp` on a NetCDF (ADR 0043). |
