#!/usr/bin/env python3
"""Provision the three SPATIAL artifacts the ADR-0039 blocked-CV experiment needs — from `grid.nc`.
...
— `x` must describe what is actually in the file — and it is stated in ADR 0039 rather than patched away.
...
GEO_COLS = ["geo_lat", "geo_lon", "geo_sin_lon", "geo_cos_lon", "geo_abs_lat", "geo_cos_lat"]
...
def write_latlon_txt(df: pl.DataFrame, meta: dict) -> Path:
    ...
    with open(path, "w") as fh:
        fh.write("# cell_latlon.txt -- per-cell orderA position, for spatially BLOCKED CV (ADR 0039).\n")
