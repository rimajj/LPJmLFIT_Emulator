...
WHAT IT PRODUCES
  ...
  (b) `test/testitems/references/S_hainich_response_boundary.csv` — COMMITTED, 1 row per (scenario, year):
...
GATES (all hard exits — the extraction is worthless if any fails)
  1. the historic W=20 boundary for 2000-2019 reproduces the committed `climbuf_hainich_boundary_w20.csv`
     ...
     than re-deriving;
  2. the historic 2010 daily row block reproduces the committed `hainich_forcing_2010.csv` — this proves the
     cell index, the YEARCELL decode, the v2/v3 scalar branch and the units all match the fixture the
     Stage-2 arm was measured on;
  3. CO2 is flat at 409.63 from 2020 (ADR 0004) — a rising ssp CO2 would mean the wrong forcing file.

Env: CELL (42490) · HIST_Y0/HIST_Y1 (1939/2019) · SSP_Y0/SSP_Y1 (2020/2100) · W (20) · OUT_DIR
...
def gate_boundary(hist_monthly, mon_y0, W):
    """GATE 1 — the historic trailing-W boundary must reproduce the committed ClimBuf fixture."""
    ref = read_ref_csv(os.path.join(REFDIR, "climbuf_hainich_boundary_w20.csv"))
    worst = 0.0
    for j, ys in enumerate(ref["year"]):
        Y = int(ys)
        iY = Y - mon_y0
        if iY < 0:
        ...
        worst = max(worst, abs(float(g[0]) - float(ref["gdd5"][j])), abs(float(t[0]) - float(ref["tas_cold_month"][j])))
    # the fixture prints a float32 with ~9 significant digits, so an exact float32 match still shows ~1e-5
    # on a gdd5 of ~1800; anything above 1e-3 is a different quantity, not a print artefact.
    print(f"== GATE 1 boundary vs climbuf_hainich_boundary_w20.csv: worst |diff| = {worst:.3g} over {len(ref['year'])} yr")
    if worst > 1.0e-3:
        raise SystemExit(f"FATAL GATE 1: trailing-W boundary does not reproduce the committed fixture ({worst:.4g})")


def gate_daily_2010(daily, hist_y0):
    """GATE 2 — the historic 2010 block must reproduce the committed `hainich_forcing_2010.csv`."""
    ...
    for v in VARS:
        got = daily[v][iY]
        want = np.array([float(x) for x in ref[v]], dtype=np.float64)
        if want.size != 365:
            raise SystemExit(f"FATAL GATE 2: fixture has {want.size} rows, expected 365")
        worst[v] = float(np.max(np.abs(got - want)))
    print("== GATE 2 daily 2010 vs hainich_forcing_2010.csv: " + "  ".join(f"{v}={worst[v]:.3g}" for v in VARS))
    ...
    if bad:
        raise SystemExit(f"FATAL GATE 2: {bad} differ from the committed 2010 forcing — wrong cell/units/decode")


def main():
    cell = int(os.environ.get("CELL", "42490"))
    ...
    # ── the boundary needs a W-1 year LEAD-IN, or the first target years get a SHORT trailing window ───────
    ...
    # reads -3.1 °C off a 1-YEAR window instead of a 20-year climatology). The ssp370 side deliberately does
    # NOT backfill from historic: its .clm starts 2015, and `build_transient_boundary.py` accepts the short
    # window for 2020-2034 as a documented edge — so replicating it is what keeps this fixture consistent with
    # the boundary table the artifacts were TRAINED against (ADR 0023).
    mon_y0 = hist_y0 - (W - 1)
    hist_lead = read_cell_years(SRC["temp"][0], cell, mon_y0, hist_y1)
    hist_monthly = monthly_means(hist_lead)
    ssp_monthly = monthly_means(daily["ssp370"]["temp"])
    print(f"== boundary lead-in: historic monthly climatology from {mon_y0} (full W={W} window from {hist_y0})")
    gate_boundary(hist_monthly, mon_y0, W)
    gate_daily_2010(daily["historic"], hist_y0)
    ...
    for scen in SCEN_IDX:
        ...
        with open(path, "w") as f:
            ...
            f.write("# Built by scripts/build_hainich_response_forcing.py (gated vs hainich_forcing_2010.csv).\n")
    ...
    ref_path = os.path.join(REFDIR, "S_hainich_response_boundary.csv")
    with open(ref_path, "w") as f:
        f.write(
            f"# Cell {cell} two-scenario ADR-0026 transient boundary (trailing W={W}) + per-year forcing means.\n"
            ...
            "# own gdd5_tcm, gated against climbuf_hainich_boundary_w20.csv.\n"
        )
        f.write("scenario,year,gdd5,tas_cold_month,co2," + ",".join(f"{v}_mean" for v in VARS) + "\n")
        for scen, monthly, m0 in (("historic", hist_monthly, mon_y0), ("ssp370", ssp_monthly, ssp_y0)):
            y0, y1 = (hist_y0, hist_y1) if scen == "historic" else (ssp_y0, ssp_y1)
            for k, Y in enumerate(range(y0, y1 + 1)):
                iY = Y - m0
                lo = max(0, iY - W + 1)
