    ...
    for line in eachline(path)
        ...
        push!(
            trees, Tree(
                ...
                gi("age"), gf("fpc"),
                gf("leaf_c") + gf("sapwood_c") + gf("heartwood_c"),
                h.total, h.hard_kill
            )
        )
    end
...
"A `FDiff.TreePools` per roster tree, so the shipped feature assembly can be called on the real stand."
...
function flux_drivers(trees::Vector{Tree})
    ...
    ws = 1.0 - sum(t.wscal_mean for t in emitted) / length(emitted)
    return (bm_inc, ge, ws)
end

"The C's OWN aggregates for the same six state columns — the training basis, logged beside the runtime one."
function ctrain_state(trees::Vector{Tree})
    ...
    agb = sum(t.agb_c for t in emitted)
    lai = sum(t.leaf_c * t.sla * t.nind for t in emitted)
...
function main(argv)
    ...
    while time() - last_seen < max_idle
        ...
        for ready in sort(filter(f -> occursin(REQ_RE, f), readdir(apply_dir; join = true)))
            ...
            ages = Float64[t.age - 1 for t in trees]
            feats = EM.flux_feature_vector(boundary, ages, npv, grow, pools, state, allom, soil)
            ...
            @printf(
                log,
                "L %d %d %d %.17g %.17g %.17g %.17g %.17g %.17g %d %d " *
