...
| 0092 | **The patch ensemble is THE central production problem, and the compute case is UNMEASURED** (owner, 2026-08-07). The emulator does not re-run the stochastic filtering — S predicts it — but the coupled driver still evaluates the daily physics on every patch, so the term that dominates runtime is unchanged from the C model. The saving is the ~1000-yr **spin-up**, not per-day cost. ⚠ **Never claim the emulator is 'faster' without saying that.** Patches are still needed for (a) comparison basis — the C's gridded truth IS a patch mean, (b) NONLINEARITY: `mean(f(state)) != f(mean(state))` (ADR 0057 measured a DENSER patch making slightly LESS carbon — water-limited GPP), (c) within-patch suppression structure. Options open: fewer / representative-stratified / effective-stand+correction / hybrid. **Status OPEN — the strategy is the owner's call, next session.** |
...
|---|---|---|---|
| **P0★ PRODUCTION PATCH STRATEGY** — the central task | 🔴 **OPEN, TOP PRIORITY** | **owner + integrator** | **THE major thing to solve (owner, 2026-08-07).** S predicts the filtering outcome, yet F still runs the daily physics on EVERY patch ⇒ the dominant cost is unchanged and the emulator's compute case rests only on skipping spin-up. 25 patches is fine for validation; **production needs a strategy** (fewer / representative / effective-stand + bias correction). 500-patch runs make it 20× worse. **Strategy to be discussed WITH the owner next session — do not pick one unilaterally.** Framing, option space + the 4 experiments: [ADR 0092](docs/decisions/0092-the-patch-ensemble-is-the-central-production-problem.md) |
